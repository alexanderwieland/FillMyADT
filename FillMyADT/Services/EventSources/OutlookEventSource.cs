...
public class OutlookEventSource : IEventSource
{
        ...
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // Use proper range filter: events that overlap with the date range
                    // This properly handles all-day events which have End set to midnight of the next day
                    // For a single day (Jan 15), we want events where:
                    //   - Start is before end of Jan 15 (i.e., before start of Jan 16)
                    //   - End is after start of Jan 15
                    var endDateNextDay = endDate.Date.AddDays(1); // Start of next day
                    var filter = $"[Start] < '{endDateNextDay:g}' AND [End] > '{startDate:g}'";
                    var restrictedItems = items.Restrict(filter);

                    Log.Debug("Outlook filter: [Start] < '{EndDateNextDay:g}' AND [End] > '{StartDate:g}'", endDateNextDay, startDate);
    ...
    /// </summary>
    private bool IsSpecialWholeDayEvent(string subject, out EventType eventType)
    {
        ...
        {
            eventType = EventType.SpecialWholeDayHomeoffice;
        ...
        {
            eventType = EventType.SpecialWholeDayHoliday;
        ...
        {
            eventType = EventType.SpecialWholeDayZeitausgleich;
